...
{
    public class ModifierAppartementCommandHandlerTest
    {
        ...
        public ModifierAppartementCommandHandlerTest()
        {
        ...
        [Fact]
        public async Task PeutModifierUnAppartementAsync()
        {
            // on cree l'appartement dans la base
            ...
            var commandModification = new ModifierAppartementCommand
            {
                Id = 1,
                Loyer = 600,
            };

            var handlerModification = new ModifierAppartementCommandHandler(_infoEcoService, _mapper, _httpContextAccessor, _verifieurReferenceService, _loggerFactory);
            await handler.Handle(command, CancellationToken.None);
            command.Loyer.Should().Be(600);
        }

        [Fact]
        public async Task NePeutModifierUnAppartementAsync()
        {
            var command = new ModifierAppartementCommand();

            var handler = new ModifierAppartementCommandHandler(_infoEcoService, _mapper, _httpContextAccessor, _verifieurReferenceService, _loggerFactory);
            await handler.Handle(command, CancellationToken.None);
